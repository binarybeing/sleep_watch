...
class VideoTransformTrack(MediaStreamTrack):
    ...
    async def recv(self):
        ...
        if alert :
            now = int(frame.pts * frame.time_base)
            if len(self.time_list) == 0 or now >= self.time_list[0] + 2 :
                self.time_list = []
                self.time_list.insert(0,now)

            elif(now!= self.time_list[0]) :
                self.time_list.insert(0,now)
                if len(self.time_list)>4:
                    self.time_list.pop()
                    rtc.dc.send("alert")
